...
### 2. Danh Sách 23 Feature Columns Chính Thức (`FEATURE_COLS`)
- Loại bỏ hoàn toàn `student_code` (ID duy nhất), `school_year_id` (hằng số), và các cột Ground Truth (`actual_final_grade`, `actual_risk_level`, `is_at_risk`).
- **Danh sách 23 Features Vector ($X$):**
  1. `evaluated_at_week` (Time Anchor)
  2. `subject_id` (Categorical Feature)
  3. `semester_index`
  ...
  22. `total_demerit_points`, 23. `repeat_offense_count` (hoặc `severe_sanction_count`).

### 3. Xử lý Categorical Features & Class Imbalance
- **Categorical Feature:** `cat_features = ['subject_id']`.
