        ...
        <div className="p-6 md:p-8 max-w-[1500px] mx-auto space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2.5">
                        ...
                        Chẩn đoán Lỗ hổng Kiến thức theo Lớp
                    </h2>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-xs flex flex-wrap items-end gap-4">
                ...
                <button
                    ...
                    className="px-5 py-2.5 rounded-xl bg-brand-600 hover:bg-brand-700 text-white text-sm font-semibold shadow-xs disabled:opacity-50 transition-all flex items-center gap-2"
                >
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs flex items-center justify-between">
                            ...
                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${rosterData.need_support_count > 0 ? "bg-rose-50 text-rose-600 dark:bg-rose-500/10 dark:text-rose-400" : "bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400"}`}>
                        ...
                        <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs flex items-center justify-between">
                            ...
                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${rosterData.cheating_alert_count + rosterData.low_engagement_count > 0 ? "bg-amber-50 text-amber-600 dark:bg-amber-500/10 dark:text-amber-400" : "bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400"}`}>
                    ...
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xs overflow-hidden space-y-4 p-5">
                                ...
                                <table className="w-full text-sm">
                                    <thead>
                                        <tr className="text-left text-xs font-bold text-slate-500 dark:text-slate-400 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/40">
                                            ...
                                            <th className="px-5 py-3.5">Độ Thành thạo TB</th>
                                            <th className="px-5 py-3.5">Tình trạng Lỗ hổng</th>
                                            <th className="px-5 py-3.5">Chương Yếu Trọng tâm</th>
                                            <th className="px-5 py-3.5">Đối soát Năng lực</th>
                                        ...
                                        {filteredStudents.map((s) => {
                                            ...
                                            return (
                                                <tr
                                                    ...
                                                    className="cursor-pointer hover:bg-brand-50/30 dark:hover:bg-slate-800/50 transition-colors group"
                                                >
                                                    {/* Cột 1: Học sinh */}
                                                    <td className="px-5 py-3.5">
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div>
                                                                <span className="font-bold text-slate-900 dark:text-slate-100 group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors block">
                                                                    {s.student_name}
                                                                </span>
                                                    ...
                                                    {/* Cột 2: Thành thạo TB với Progress bar */}
                                                    <td className="px-5 py-3.5">
                                                    ...
                                                    {/* Cột 3: Tình trạng lỗ hổng */}
                                                    <td className="px-5 py-3.5">
                                                        {s.gap_count > 0 ? (
                                                            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-rose-50 text-rose-700 border border-rose-200 dark:bg-rose-500/10 dark:text-rose-300 dark:border-rose-500/20">
                                                                <Target className="w-3.5 h-3.5 text-rose-500" />
                                                                {s.gap_count} bài cần củng cố
                                                            </span>
                                                        ) : (
                                                            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-300 dark:border-emerald-500/20">
                                                                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" />
                                                                Vững toàn bộ {s.total_units} bài
                                                            </span>
                                                        )}
                                                    </td>

                                                    {/* Cột 4: Chương yếu trọng tâm */}
                                                    <td className="px-5 py-3.5">
                                                        {s.weak_units.length > 0 ? (
                                                            <div className="flex items-center gap-1.5 flex-wrap">
                                                                {s.weak_units.map((u, i) => (
                                                                    <span
                                                                        key={i}
                                                                        className="px-2 py-0.5 rounded-md bg-rose-50 text-rose-700 dark:bg-rose-950/40 dark:text-rose-300 text-xs font-medium border border-rose-100 dark:border-rose-900/40"
                                                                    >
                                                                        {u}
                                                                    </span>
                                                                ))}
                                                    ...
                                                    <td className="px-5 py-3.5">
                                                        <div className="flex items-center gap-2 flex-wrap">
                                                            <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-semibold border ${integ.cls}`}>
                                                                {integ.icon}
                                                                {integ.label}
                                                            </span>
                                                            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold border ${conf.cls}`}>
                                                                <span className={`w-1 h-1 rounded-full ${conf.dotCls}`} />
                                                                {conf.label}
                                                            </span>
                                                        </div>
                                                    </td>
