...
[SCHEMA: s360 - EWS (EARLY WARNING SYSTEM / DỰ ĐOÁN RỦI RO HỌC TẬP)]
25. Bảng `s360.fact_student_subject_risk_predictions`: Kết quả dự đoán rủi ro học tập (EWS) theo từng học sinh - môn học - mốc tuần.
    - Các cột định danh: `id` (BIGINT PK), `student_code` (VARCHAR), `so_school_id` (INT - Mã trường), `subject_id` (FK -> s360.dim_subject.id), `school_year_id` (FK -> s360.dim_school_year.id), `semester_index` (1 hoặc 2), `evaluated_at_week` (INT - Mốc tuần cắt dữ liệu), `model_version` ('v1_single' hoặc 'v2_ensemble'), `cutoff_date` (DATE), `evaluated_at_date` (DATE).
    - Các cột điểm rủi ro: `risk_score` (DECIMAL 0-100 - Điểm rủi ro tổng hợp), `risk_level` (VARCHAR - Mức rủi ro: 'LOW', 'MODERATE', 'HIGH', 'CRITICAL'), `risk_probability` (DECIMAL 0-1 - Xác suất rủi ro), `score_risk`, `lms_risk`, `attendance_risk`, `behavior_risk` (Điểm rủi ro từng yếu tố), `weight_score`, `weight_lms`, `weight_attendance`, `weight_behavior` (Trọng số từng yếu tố).
    - Các cột feature: `weighted_early_avg`, `weighted_late_avg`, `weighted_late_avg_imputed` (BOOLEAN), `score_slope`, `score_volatility`, `max_drop`, `last_score`, `max_coefficient_so_far`, `high_weight_score_count`, `last_high_weight_score`, `lms_avg_score`, `lms_recent_drop`, `lms_submission_rate`, `lms_recent_submission_rate`, `lms_gradebook_gap`, `daily_absence_rate`, `unexcused_absent_rate`, `excused_absent_days`, `total_late_count`, `total_demerit_points`, `repeat_offense_count`, `severe_sanction_count`.
    - LƯU Ý: Bảng này KHÔNG có cột `grade_id`/`homeroom_class_id`/`subject_name` trực tiếp. Để lấy TÊN MÔN HỌC hãy JOIN `s360.dim_subject sub ON rp.subject_id = sub.id` (lấy `sub.name`); để lấy TÊN HỌC SINH / LỚP / KHỐI hãy JOIN `s360.dim_homeroom_class_student st ON rp.student_code = st.student_code` (lấy `st.student_name`, `st.class_name`, `st.grade_id`). Bộ lọc `so_school_id` được hệ thống tự động chèn, KHÔNG cần (và KHÔNG được) tự thêm điều kiện `so_school_id` vào WHERE.
    - Ví dụ: đếm học sinh theo mức rủi ro: `SELECT rp.risk_level, COUNT(DISTINCT rp.student_code) FROM s360.fact_student_subject_risk_predictions rp WHERE rp.school_year_id = 2025 AND rp.semester_index = 1 AND rp.evaluated_at_week = 8 AND rp.model_version = 'v1_single' GROUP BY rp.risk_level`.
