...
class RouterDecision(BaseModel):
    next_agent: str = Field(
        description=(
            "Chọn agent tiếp theo cần gọi: 'data_service_agent' (cho TẤT CẢ tra cứu CSDL, thông tin/điểm số thô, bảng điểm cá nhân, bảng điểm lớp, truy vấn thống kê toàn khối lớp, phân tích thống kê/chỉ số sâu GDI, Delta G, Momentum, độ khó đề thi EDI/CDI, VÀ phân tích dự báo rủi ro học tập EWS — học sinh có nguy cơ trượt/sụt giảm, mức rủi ro LOW/MODERATE/HIGH/CRITICAL, điểm rủi ro risk_score, danh sách học sinh rủi ro theo lớp/khối/môn), "
...
3. `data_service_agent` (DỰ BÁO RỦI RO HỌC TẬP EWS): Mọi câu hỏi về dự báo/cảnh báo rủi ro học tập — học sinh có nguy cơ trượt/sụt giảm, mức rủi ro (LOW/MODERATE/HIGH/CRITICAL), điểm rủi ro (risk_score), xác suất rủi ro, danh sách học sinh rủi ro theo lớp/khối/môn, phân tích yếu tố rủi ro (điểm số, LMS, chuyên cần, hạnh kiểm) — đều định tuyến sang `data_service_agent` (truy vấn bảng `s360.fact_student_subject_risk_predictions`). KHÔNG định tuyến sang `knowledge_agent` hay `report_agent` cho loại câu hỏi này.
4. `data_service_agent` (LỖ HỔNG KIẾN THỨC & DỰ ĐOÁN PASS/FAIL ĐỀ CUỐI KỲ):
   - Mọi câu hỏi về "học sinh hổng kiến thức gì", "học sinh yếu chương/bài nào", "cần ôn tập phần nào" → định tuyến sang `data_service_agent` gọi công cụ `get_knowledge_gap_report`.
   - Mọi câu hỏi về "dự đoán bao nhiêu em trượt/pass", "tỉ lệ đậu/rớt đề cuối kỳ", "bao nhiêu học sinh đạt môn X" → định tuyến sang `data_service_agent` gọi công cụ `get_pass_fail_forecast`.
5. `knowledge_agent`: Chuyên trách tra cứu NỘI DUNG KIẾN THỨC trong sách giáo khoa (định nghĩa, công thức, khái niệm, giải thích bài học của các môn Toán, Ngữ Văn, KHTN, Sử-Địa...). Dùng cho câu hỏi về nội dung học thuật, KHÔNG dùng cho điểm số/hồ sơ học sinh.
6. `report_agent`: Chuyên trách tổng hợp số liệu báo cáo, tạo bảng hiển thị xem trước báo cáo thống kê, hoặc cung cấp đường link tải xuống các tệp báo cáo thống kê (Word, HTML/PDF). Hãy gọi agent này khi người dùng yêu cầu xuất báo cáo, tải báo cáo, lập báo cáo, hoặc khi muốn xem số liệu tổng quan của một loại báo cáo cụ thể.
7. `CLARIFICATION`: Chọn trạng thái này khi bạn cần hỏi lại người dùng để làm rõ thông tin thiếu (như thiếu năm học, thiếu học kỳ, mơ hồ lớp học) HOẶC khi câu hỏi chỉ là chào hỏi xã giao bình thường. Bắt buộc viết nội dung phản hồi trực tiếp vào trường `response`.
8. `FINISH`: Chọn trạng thái này KHI VÀ CHỈ KHI bạn đã thu thập đầy đủ thông tin từ các Sub-Agent để tổng hợp trả lời câu hỏi gốc của người dùng.
