...
def get_embedding(text_input: str) -> list[float] | None:
    """Sinh vector embedding cho chuỗi văn bản sử dụng Provider đã cấu hình (Gemini REST API hoặc OpenAI)."""
    ...
    try:
        if settings.embedding_provider == "gemini" and settings.gemini_api_key:
            ...
            payload = {
                ...
                "content": {"parts": [{"text": text_input.strip()}]},
            }
            ...
            return embedder.embed_query(text_input)
